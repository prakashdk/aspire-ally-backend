...
export default class GoalRepository {
  static getShortTermGoals = async ({ goal, count }) => {
    ...
    const prompt = `Give me short-term goals to achieve ${goal} as JSON with id - a random alphanumeric uuid, topics, actions and timeSpan as keys and add approximate time span to every short term goals. The actions should be self-explanatory. assign the final JSON output to a key shortTermGoals. The final response should be in JSON format, Don't add additional text`;
